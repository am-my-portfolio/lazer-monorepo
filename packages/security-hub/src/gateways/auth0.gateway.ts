...
export class UserRequests {
  ...
  static orchestrateUserCreation = async (user: ICreateUser, auth0_org_id: string, access_token?: string) => {
    ...
    const { data: auth0_user } = await UserRequests.createAuth0User(user, access_token);
    if (!auth0_user) {
      throw new Error('Failed to create user in auth0');
    }

    await OrganizationRequests.addMembersToOrganization(auth0_org_id, auth0_user.user_id, access_token);
    ...
    await UserRequests.inviteToPortal({ email, auth0_org_id, client_id }, access_token);
  ...
  static inviteToPortal = async (body: ISendPortalInviteRequest, access_token?: string) => {
  ...
  static getAllUserDetails = async (access_token: string, user_type: AllRoles) => {
    const url = '';
    const config = {
      url,
      method: 'GET',
  ...
  static getUsersWithSameEmail = async (access_token: string, user_id: string, email: string) => {
    const config = {
...
export class OrganizationRequests {
  static orchestrateTenantOnboarding = async (team_details: any) => {
    const { display_name, given_name, family_name, email, logo_url } = team_details;
    ...
    const auth0_org: IAuth0Organization = {
      ...
      enabled_connections,
      branding: {
        logo_url: logo_url ? logo_url : 'https://acmebusiness.com/wp-content/uploads/2020/07/ACME-Logo-CMYK-pdf.jpg',
        colors: {
          primary: '#dc2626',
    ...
    const { data, error } = await OrganizationRequests.createAuth0Organization(auth0_org, access_token);
    if (error) {
  ...
  static createAuth0Organization = async (body: IAuth0Organization, access_token?: string) => {
  ...
  static addMembersToOrganization = async (auth0_org_id: string, auth0_member_id: string, access_token?: string) => {
