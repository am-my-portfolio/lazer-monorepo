    ...
    <li v-for="item in items" :key="item.name">
      <details>
        <summary class="flex max-w-xs my-1 text-left group">
          <div class="items-center flow-root w-full gap-0 py-1 rounded-md cursor-pointer">
            <div class="float-left" :class="item.items.length === 0 ? 'w-full' : 'w-[90%]'">
              <a
                :href="item.target"
                :class="[
                  item.current
            ...
            <div class="float-right" v-if="item.items && item.items.length > 0">
              <i class="fa-solid fa-chevron-down mt-3 text-pop-secondary" aria-hidden="true"></i>
            </div>
          </div>
        </summary>
        <div>
