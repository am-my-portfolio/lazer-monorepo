<template>
  <PageDivisionLayout>
    ...
    <template #divisionDescription> Monitor and manage AI model interactions </template>
            ...
            <tbody>
              <template v-for="day in statuses" :key="day.dateTime">
                ...
                <tr v-for="transaction in day.transactions" :key="transaction.id">
                  <td class="hidden relative py-5 pr-6 sm:table-cell">
                    <div class="flex gap-x-6">
                      <img :src="transaction.imageUrl" class="h-12 rounded-full" />
                      <div class="flex-auto">
                        <div class="text-sm leading-6 text-pop-secondary">
                          {{ transaction.name }}
                        </div>
                        <div class="mt-1 text-xs leading-6 text-pop-primary">
                          {{ transaction.role }}
                        </div>
                      </div>
                    </div>
