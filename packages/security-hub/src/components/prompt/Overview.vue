<template>
  <PageDivisionLayout>
    <template #divisionTitle>Chatbot </template>
    <template #divisionDescription>
      <p class="text-sm text-pop-primary">Powered by LAZER.io</p>
    </template>
    <template #divisionContent>
      ...
      <div class="grid grid-col-1 md:grid-cols-3 xl:grid-cols-5 font-mono text-md">
        <div class="col-start-1 md:col-start-1 md:col-end-4 xl:col-start-1 xl:col-end-6">
          <div class="mt-10 bg-pop-secondary/70 p-6 rounded-lg border border-pop-primary">
            <!-- Chat Container -->
            <div class="h-[50vh] overflow-y-auto scrollbar">
              <div v-for="item in conversation">
                <div id="question" class="place-self-start">
                  ...
                  <div class="flex flex-1 gap-3 my-4 text-primary">
                  ...
                  <div class="flex flex-1 gap-2 my-4">
                    ...
                    <span class="relative flex shrink-0 overflow-hidden rounded-full w-8 h-8">
                      <div class="rounded-full bg-gray-100 border px-1.5 py-1">
                        <i class="fa-solid fa-wand-magic-sparkles text-pop-primary"></i>
                      </div>
                    </span>
                  </div>
                </div>
            ...
            <div class="relative flex flex-col my-4">
              <input
                ...
                @keyup.enter.native="chatSSE()"
                class="flex h-20 w-full rounded-md border border-secondary px-3 py-2 placeholder-pop-primary/50 text-pop-primary focus:outline-none focus:ring-2 disabled:cursor-not-allowed disabled:opacity-50 focus-visible:ring-offset-2"
              />
...
import { useDocuments } from '@/composables/private-gpt/useDocuments';
import { useRecipes } from '@/composables/private-gpt/useRecipes';
// import { useChat } from '@/composables/private-gpt/useChat';
// import { usePgptHealth } from '@/composables/private-gpt/usePgptHealth';
...
const chatSSE = async () => {
  ...
  const eventSource = new EventSource(`${base_url}/${provider}/completion/${question.value}`);
...
//   const stream = await useChat(new_entry.value.question, 'openai');
...
const ingestDocument = async () => {
  const { ingest } = useDocuments();

  files.value = file_input.value?.files;
  const file = file_input.value.files[0];

  const new_entry = ref({
    question: `ingest document ${file.name}`,
    answer: undefined,
  });
  question.value = undefined;
  conversation.value.push(new_entry.value);
...
<!-- inspiration: https://www.creative-tim.com/twcomponents/component/ai-chatbot -->
