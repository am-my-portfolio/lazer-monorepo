...
export const applyPermissionRouteGuard: NavigationGuard = (to, from, next) => {
  ...
  if (to.meta.has_any_permissions) {
    permission_check = required_permissions.some((permission) => user_permissions?.includes(permission));
  } else {
    permission_check = required_permissions.every((permission) => user_permissions?.includes(permission));
  }
