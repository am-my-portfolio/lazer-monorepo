...
import { cors_allowed_headers, cors_config, cors_max_age, cors_methods, Environment } from './types';
...
async function bootstrap() {
  const app_id = 'LAZER AI';
  const logger = new Logger(`${app_id} Service`);
  const app = await NestFactory.create(AppModule, {
    bufferLogs: true,
  });
  const configService = app.get(ConfigService);
  const port = configService.get('APP_AI_SERVICE_PORT') || 3000;
